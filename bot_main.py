...
def text_to_speech_simple(text: str, user_id: int, lang: str = "ru") -> str:
    """
    Простое преобразование текста в .mp3 файл
    """
    try:
        # Ограничиваем длину
        if len(text) > 900:
            text = text[:900] + " [сокращено]"

        # Путь к файлу
        filename = os.path.join(AUDIO_DIR, f"audio_{user_id}.mp3")
...
def main_keyboard():
    ...
    builder.add(
        ...
        types.KeyboardButton(text="🔈 Озвучить")
    )
...
class Form(StatesGroup):
    weather = State()
    translate = State()
    voice = State()  # ← новое состояние

# --- Хендлеры
#
@dp.message(F.text == "🔈 Озвучить")
async def ask_for_text_to_speak(message: Message, state: FSMContext):
    await state.set_state(Form.voice)
    await message.answer("Введите текст, который хотите озвучить:", reply_markup=cancel_keyboard())


@dp.message(Form.voice)
async def send_speech(message: Message, state: FSMContext):
    ...
    # Преобразуем в речь
    audio_path = text_to_speech_simple(text, user_id, lang="ru")
    if not audio_path:
    ...
    # Отправляем как аудиофайл (не голосовое, но воспроизводится)
    try:
        audio = FSInputFile(audio_path)
        await message.answer_audio(audio, caption="🔈 Ваше сообщение:")
    except Exception as e:
